...
my $long=@ARGV;
if ($long != 7){
    print "Usage: perl $0 <title> <ini> <end> <ini2> <end2> <offset> <outFile>\n";
    exit(0);
}

my ($title,$ini,$end,$ini2,$end2,$offset,$out)=@ARGV;
...
my $pos2 = int(($end2 - $ini2) / $offset) + 1;
my $endR = $end - 1;
#my $strand = $end/2;
...
print OUT "MEAN <- matrix(nrow = $pos, ncol = $pos2)\n";
print OUT "STDEV <- matrix(nrow = $pos, ncol = $pos2)\n";
print OUT "MIN <- matrix(nrow = $pos, ncol = $pos2)\n";
print OUT "MAX <- matrix(nrow = $pos, ncol = $pos2)\n";
#print OUT "p <- $pos-1\n";
#print OUT "pp <- $pos2\n";
#print OUT "conti <- $ini\n";
print OUT "for (i in $ini:$end){\n";
...
print OUT "\tindexi <- i - $ini + 1\n";
print OUT "\tfor (j in $ini2:$end2){\n";
print OUT "\tindexj <- j - $ini2 + 1\n";
#print OUT "\t\tindex1 <- i + $conti\n";
#print OUT "\t\tindex2 <- j + $contj\n";
#print OUT "\t\tmyfile <- paste(conti,\"-\",contj,\".dat\",sep=\"\")\n";
print OUT "\t\tmyfile <- paste(i,\"-\",j,\".dat\",sep=\"\")\n";
#print OUT "\t\tcontj <- contj + $offset\n";
print OUT "\tif (file.exists(myfile)){\n";
print OUT "\t\tinteract <- read.table(myfile)\n";
print OUT "\t\tMEAN[indexi,indexj]= sapply (interact[2],mean)\n";
#print OUT "\t\tMEAN[indexj,indexi]= sapply (interact[2],mean)\n";
print OUT "\t\tSTDEV[indexi,indexj]= sapply (interact[2],sd)\n";
#print OUT "\t\tSTDEV[indexj,indexi]= sapply (interact[2],sd)\n";
print OUT "\t\tMIN[indexi,indexj]= sapply (interact[2],min)\n";
#print OUT "\t\tMIN[indexj,indexi]= sapply (interact[2],min)\n";
print OUT "\t\tMAX[indexi,indexj]= sapply (interact[2],max)\n";
#print OUT "\t\tMAX[indexj,indexi]= sapply (interact[2],max)\n";
print OUT "\t}\n";
...
#print OUT "axis(1, xat, at = 0.5:1:xdim[1],las=2)\n";
#print OUT "axis(2, yat, at = 0.5:1:xdim[2],las=2)\n";
