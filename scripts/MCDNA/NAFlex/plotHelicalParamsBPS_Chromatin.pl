...
my $long=@ARGV;
if ($long != 2){
    print "Usage: perl $0 <curvesAvg> <BasePairStepParamsAvg.table>\n";
    exit(0);
}
...
while(<AVG>){
	...
	print GNUP "$pair $value $stdev $avgParm $avgParmMD $avgParmRNA\n";
...
if($type eq "DNA"){
print OUT "plot \"$avg_input.gnuplot\" using 0:2:3:xticlabels(1) with yerrorlines ls 1 notitle,  \"$avg_input.gnuplot\" using 2:xticlabels(1) with lp ls 1 title '$title User-MD Average', \"$avg_input.gnuplot\" using 5:xticlabels(1) with lp ls 2 title '$title ABC Average', \"$avg_input.gnuplot\" using 4:xticlabels(1) with lp ls 3 title '$title X-Ray Average'\n";
} else{
