...
sub run_distContactMapsPDB{

	my ($workdir,$prefix,$pdb,$top) = @_;
	...
	`perl $scriptsDir/pdb2crd.pl $pdb jc.crd`;
	my $crd = "jc.crd";
	...
	my $end = $l;
	my $offset = 1;
	print "perl $scriptsDir/plotContactMap_R.pl $title $ini $end $offset $out\n";

	`perl $scriptsDir/plotContactMap_R.pl "$title" $ini $end $offset $out > distContMap.R.out 2>&1`;
...
#		$done{$d} = 1;
	...
	`$amberexe/cpptraj $top < ptraj.in > ptraj.out 2>&1`;
