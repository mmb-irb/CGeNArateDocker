...
void ToTetramersLinear(char *RNAME, int nbp, int *TNums)
{
	/*Reads the bases RNAME associated with each particle, and returns the Tetramer identifier TNums associated with
	 each grup of 4 nucleotides. nbp is the number of basePairs. terminal tetramers assume a phantom G5. Note: this code
	 reads ONLY the Watson strand, and therefore assumes canonical basepairs A-T, C-G*/
	int i, k;
	int aux;
	int num;
								 ...
								 double KLong[4][Order])
{
	...
	There are n particles, with positions r and forces F, with n/2-1 tetramer identifiers (special behavior for terminal
	bp) The dictionaries of Distances and Constants are DTetramers and KTetramers for the 4mer region, and Dlong and
	Klong for the distant region, nn is the maximum bonded interaction, nn=5*/
	...
	#pragma omp single label("TLin-linker-init")
	{
		memset(F, 0, sizeof(double) * n * 3); // linear-1
	...
	for (int skipConnections = 0; skipConnections < skipSize; skipConnections++)
		#pragma omp for schedule(static) label("TLin-linker-main")
	...
	#pragma omp single label("TLin-linker-end")
	{
		/*End*/
		i = n / 2 - 2;
		/*Bond, Bond-*/
						   ...
						   double KLong[4][Order], double *const E)
{
	...
	There are n particles, with positions r, with n/2-1 tetramer identifiers (special behavior for terminal bp)
	...
	/*Beginning*/
	#pragma omp single label("TLin-energy-init")
	{
		i = 0;
		*E = 0;
		double tE = 0;
	...
	/*Tetramers*/
	#pragma omp for schedule(static) reduction(+:E[0:1]) label("TLin-energy-main")
	for (i = 1; i < n / 2 - 2; i++)
	{
		double tE = 0.;
		/*Bond, Bond-*/
	...
	#pragma omp single label("TLin-energy-end")
	{
		/*End*/
		i = n / 2 - 2;
		double tE = 0;
